...
function montarPagina(livros, agora = new Date()) {
  const cards = livros.map(l => `
    <a class="card" href="${esc(l.link)}" target="_blank" rel="noopener sponsored">
      <img loading="lazy" src="img/${esc(l.imagem)}" alt="Capa do e-book ${esc(l.titulo)}" width="300" height="480">
      <div class="info">
        <h2>${esc(l.titulo)}</h2>
        <p>${esc(resumo(l.descricao))}</p>
        <div class="row"><span class="preco">${esc(precoBR(l.preco))}</span><span class="btn">Comprar na Hotmart</span></div>
      </div>
    </a>`).join('');
  const ld = {
...
async function main() {
  ...
  for (const d of destaques) {
    ...
    livros.push({ titulo: d.title, descricao: d.description, preco: p.price && p.price.value, link: p.salesPageLink, imagem });
  }
  ...
  console.log(JSON.stringify({ livros: livros.length, pagina: BASE_URL }));
}

module.exports = { montarPagina, resumo, precoBR, esc, comUrlNoSitemap };
