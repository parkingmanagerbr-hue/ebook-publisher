...
const DISCO = {
  ...
  criar(arquivo) {
    try { fs.closeSync(fs.openSync(arquivo, 'wx')); return true; }
    catch (e) {
...
function travar(nome, opcoes = {}) {
  ...
  if (!disco.criar(arquivo)) {
    ...
    if (nascida != null) avisar('trava de "' + nome + '" abandonada ha ' + Math.round(idade / 60000) + ' min — assumindo');
    disco.remover(arquivo);
    // Outro processo pode ter assumido no mesmo instante: quem perder, espera.
    if (!disco.criar(arquivo)) return null;
  }
...
module.exports = { travar, caminhoDaTrava, VALIDADE_MS, DISCO };
